...
async def display_ready_status():
    """
    Continuously checks the number of players who have readied up and prints a message to show how many players are left to ready up.
    """
...
async def wait_for_players(websocket, path):
    """
    Registers new players and waits for them to get ready to start playing. Once enough players are ready, it starts the game and receives songs from the players and their votes for the chosen song.
    """
...
async def run_game():
    """
    Runs the game logic, selecting a random song and player, sending the song to all players, receiving their votes, and updating the scores accordingly. It continues until one player has reached a score of 10.
    """
...
start_server = websockets.serve(wait_for_players, "0.0.0.0", port, ping_timeout=300)
